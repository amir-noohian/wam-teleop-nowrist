...
template <size_t DOF> class FollowerPi0 : public barrett::systems::System {
  ...
  public:
    ...
    void switchToPolicyRollouts() {
        ...
        policyOutputValue->setData(&tmp_jp); // initialize the policy output to the current position to
  ...
  protected:
    ...
    virtual void operate() {
        ...
        boost::optional<ReceivedData> policy_received_data = policy_handler.getLatestReceived();
        ...
        case State::ROLLOUTS:
            ...
            control = compute_control_finetune(policyJp, policyJv, policyTorque, theirJp, theirJv, theirExtTorque,
  ...
  private:
    ...
    jt_type compute_control_finetune(const jp_type &policy_pos, const jv_type &policy_vel, const jt_type &policy_torque,
                                     const jp_type &ref_pos, const jv_type &ref_vel, const jt_type &ref_extTorque,
    ...
    jt_type compute_control(const jp_type &ref_pos, const jv_type &ref_vel, const jt_type &ref_extTorque,
        ...
        jt_type u4 = -0.5 * ref_extTorque + cur_dyn - cur_grav; // PF-PF with ref external torque feedback and
                                                                // dynamic compensation (Lawrence's perfect
                                                                // transparency architecture);

        jt_type u5 = -0.5 * ref_extTorque - 0.15 * (ref_extTorque + cur_extTorque); // PF-PF with ref external torque
                                                                                    // and cur external torque feedback

        jt_type u6 = -0.5 * ref_extTorque - 0.15 * (ref_extTorque + cur_extTorque) + cur_dyn -
                     cur_grav; // it has the best performance
