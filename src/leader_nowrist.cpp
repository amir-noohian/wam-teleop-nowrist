...
// This a version of 4-DOF leader without haptic wrist (the haptic wrist is
...
template <size_t DOF> int wam_main(int argc, char **argv, ProductManager &pm, systems::Wam<DOF> &wam) {
    ...
    // NOTE Amir doesn't use filters - but could try them.
    ...
    // NOTE: if we use filters above we should also change here.
    if (use_dynamics_for_ext_torque) {
    ...
    while (going) {
        ...
        case 'g':
            if (leader.isLinked()) {
                ...
                // we rely on revious follower to leader linking to move them together
